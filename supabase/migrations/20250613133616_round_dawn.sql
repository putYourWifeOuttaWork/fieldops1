...
COMMENT ON FUNCTION share_submission_session IS 'Shares or escalates a submission session based on the provided action_type. "share" action always sets status to Shared (unless already Escalated). "escalate" action always sets status to Escalated.';