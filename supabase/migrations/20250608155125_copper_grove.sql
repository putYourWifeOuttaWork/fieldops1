/*
  # Add Static Site Properties
  
  1. New ENUMs
    - primary_function_enum: For storing the main purpose of the site
    - construction_material_enum: For the primary building material
    - insulation_type_enum: For insulation quality
    - hvac_system_type_enum: For HVAC system categorization
    - irrigation_system_type_enum: For irrigation system categorization
    - lighting_system_enum: For the lighting configuration
    - vent_placement_enum: For the position of vents

  2. New Columns
    - Adds multiple columns to sites table for storing facility attributes
    - Adds constraints for numeric fields to ensure valid ranges
    - All fields are nullable to maintain backward compatibility

  3. Updated Functions
    - Updates site creation and update functions to handle new properties
*/

-- 1. Create new ENUM types for site properties
...
-- Vent placement options (reusing the 9-point matrix concept)
...
-- 2. Add new columns to the sites table
ALTER TABLE sites 
-- Physical attributes
ADD COLUMN square_footage NUMERIC CHECK (square_footage >= 100 AND square_footage <= 1000000000),
ADD COLUMN cubic_footage NUMERIC CHECK (cubic_footage >= 25 AND cubic_footage <= 1000000),
ADD COLUMN num_openings INTEGER CHECK (num_openings >= 1 AND num_openings <= 100),
ADD COLUMN num_vents INTEGER CHECK (num_vents >= 1 AND num_vents <= 10000),
ADD COLUMN vent_placements vent_placement_enum[],

-- Facility details
ADD COLUMN primary_function primary_function_enum,
ADD COLUMN construction_material construction_material_enum,
ADD COLUMN insulation_type insulation_type_enum,

-- Environmental controls
ADD COLUMN hvac_system_present BOOLEAN DEFAULT FALSE,
ADD COLUMN hvac_system_type hvac_system_type_enum,
ADD COLUMN irrigation_system_type irrigation_system_type_enum,
ADD COLUMN lighting_system lighting_system_enum;
...
-- 4. Create an RPC function to update site properties
