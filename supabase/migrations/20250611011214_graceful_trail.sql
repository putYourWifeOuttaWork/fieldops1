/*
  ...
  1. Changes
    - Updates cancel_submission_session function to delete associated observations
    - Preserves the main submission record
    - Removes cancelled sessions from visibility in UI
    
  2. Purpose
    - Ensures proper cleanup when a session is cancelled
    - Maintains data integrity by keeping the submission record
*/

-- Drop the existing function
DROP FUNCTION IF EXISTS cancel_submission_session;

-- Create an improved version that deletes associated observations
...
COMMENT ON FUNCTION cancel_submission_session IS 'Cancels a submission session, deletes associated observations, but preserves the main submission record';