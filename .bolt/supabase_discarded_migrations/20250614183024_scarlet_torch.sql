/*
  ...
  1. Changes
    - Creates ventilation_strategy_enum type
    - Adds ventilation_strategy column to sites table
    - Updates relevant functions to handle the new field
    - DOES NOT modify the existing airflow column in submissions table
    
  2. Purpose
    - Adds more detailed ventilation strategy data at the site level
    - Provides enhanced environmental tracking capabilities
    - Maintains backward compatibility with existing airflow usage
*/
...
-- 2. Add ventilation_strategy column to sites table if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'sites' AND column_name = 'ventilation_strategy'
  ) THEN
...
-- 3. Update create_site_without_history function to handle ventilation_strategy
...
BEGIN
  ...
  -- Insert the new site with all properties including ventilation_strategy
  INSERT INTO sites (
    name, 
    type, 
    program_id, 
...
-- 4. Update update_site_properties function to handle ventilation_strategy
